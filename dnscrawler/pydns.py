...
class PyDNS(AsyncContextManager):
    def __init__(
        ...
        super().__init__()
        self.socket_factories = [socket.socket] + [self.create_socket_factory(
            factory['addr'], factory['port']) for factory in socket_factories]
        ...
        self.MAX_CACHED_QUERIES = MAX_CACHED_QUERIES or \
            constants.MAX_CACHED_QUERIES
        self.MAX_CONCURRENT_REQUESTS = MAX_CONCURRENT_REQUESTS or \
            constants.MAX_CONCURRENT_REQUESTS
        self.MAX_REQUESTS_PER_NAMESERVER_SECOND = MAX_REQUESTS_PER_NAMESERVER_SECOND or \
            constants.MAX_REQUESTS_PER_NAMESERVER_SECOND
        self.MAX_REQUESTS_PER_TLD_NAMESERVER_SECOND = MAX_REQUESTS_PER_TLD_NAMESERVER_SECOND or \
            constants.MAX_REQUESTS_PER_TLD_NAMESERVER_SECOND
    ...
    async def __aenter__(self):
        self.concurrent_request_limiter = asyncio.Semaphore(
    ...
    async def calculate_rps(self):
        ...
        self.max_requests_per_second = max(
            self.max_requests_per_second, current_requests_per_second)
        self.min_requests_per_second = min(
            self.min_requests_per_second, current_requests_per_second)
        self.avg_requests_per_second = (
            self.request_measurement_count * self.avg_requests_per_second +
